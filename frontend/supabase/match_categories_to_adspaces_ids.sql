-- ============================================
-- MATCH CATEGORIES TO AD SPACES - SET PARENT_CATEGORY_ID TO AD_SPACE IDs
-- ============================================
-- This script:
-- 1. Drops the existing foreign key constraint
-- 2. Sets all parent_category_id to NULL first
-- 3. Updates parent_category_id to reference ad_spaces.id
-- 4. Creates new foreign key constraint to ad_spaces

-- Step 1: Drop existing foreign key constraint (if it references categories)
ALTER TABLE categories
DROP CONSTRAINT IF EXISTS categories_parent_category_id_fkey;

-- Step 2: First, set all parent_category_id to NULL to clear any category references
UPDATE categories
SET parent_category_id = NULL;

-- Step 3: Update parent_category_id to reference the first ad space for each category
-- Only update categories that have ad spaces
UPDATE categories c
SET parent_category_id = (
  SELECT a.id 
  FROM ad_spaces a 
  WHERE a.category_id = c.id 
  ORDER BY a.created_at ASC 
  LIMIT 1
)
WHERE EXISTS (
  SELECT 1 
  FROM ad_spaces a 
  WHERE a.category_id = c.id
);

-- Step 4: Create new foreign key constraint to ad_spaces
ALTER TABLE categories
ADD CONSTRAINT categories_parent_category_id_fkey 
FOREIGN KEY (parent_category_id) 
REFERENCES ad_spaces(id)
ON DELETE SET NULL;

-- ============================================
-- VERIFY THE UPDATES
-- ============================================

-- View categories with their matched ad spaces
SELECT 
  c.id as category_id,
  c.name as category_name,
  c.parent_category_id as ad_space_id,
  a.title as ad_space_title,
  a.description as ad_space_description
FROM categories c
LEFT JOIN ad_spaces a ON c.parent_category_id = a.id
ORDER BY c.name;

-- View summary: categories with ad space matches
SELECT 
  c.name as category_name,
  COUNT(DISTINCT a.id) as total_ad_spaces_in_category,
  c.parent_category_id as matched_ad_space_id,
  CASE 
    WHEN c.parent_category_id IS NOT NULL THEN '✅ Matched'
    ELSE '❌ No match'
  END as match_status
FROM categories c
LEFT JOIN ad_spaces a ON a.category_id = c.id
GROUP BY c.id, c.name, c.parent_category_id
ORDER BY c.name;

-- View categories that have ad spaces but weren't matched
SELECT 
  c.name as category_name,
  COUNT(a.id) as ad_space_count
FROM categories c
INNER JOIN ad_spaces a ON a.category_id = c.id
WHERE c.parent_category_id IS NULL
GROUP BY c.id, c.name
ORDER BY c.name;

-- ============================================
-- ALTERNATIVE: If you want to keep both relationships
-- ============================================
-- You could add a new column instead:
-- ALTER TABLE categories ADD COLUMN representative_ad_space_id UUID REFERENCES ad_spaces(id);
-- Then update that column instead of parent_category_id

